...
// also after I made roundInt, I realized I could have converted all values to
// cents and not needed all this complex logic. oops.
...
func MakeAccount(name string, initialBalance string) (bankAccount, error) {
	...
	if _, err := parseDollars(initialBalance); err != nil {
		...
		if initialBalance[0] == '-' {
			fmt.Println("[WARNING] Attempting to open account for ", account.CheckName(), " with a negative initial balance of ", initialBalance)
...
func JointAccount(account1 *bankAccount, account2 *bankAccount) (bankAccount, error) {
	...
	jointAccount.ChangeName(fmt.Sprint("JOINT: ", account1.CheckName(), " & ", account2.CheckName()))
...
func (account *bankAccount) Withdraw(amount string) {
	...
	if totalToWithdraw > totalInAccount {
		err := fmt.Errorf("amount to withdraw (%s) exceeds available funds in account: %s", amount, account.CheckName())
		fmt.Println("[ERROR] Withdrawal unsuccessful.")
		fmt.Println("[ERROR] Error details: ", err)
	} else {
		newCents := int64(account.balance.cents - parsedAmount.cents)
...
// ChangeOwnerName changes the name of the owner of the bankAccount
func (account *bankAccount) ChangeOwnerName(newOwnerName string) {
	account.ChangeName(newOwnerName)
}
