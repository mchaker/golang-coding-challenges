...
import (
	...
	"github.com/mchaker/golang_coding_challenges/go_challenge4/challenge3/bank"
	"github.com/mchaker/golang_coding_challenges/go_challenge4/challenge3/customer"
)
...
func main() {
	...
	fmt.Println("----- ChangeName tests")
	testTitle = "=== Test: Change an account owner name on a bankAccount object"
	newName = "New JointAccountName"
	fmt.Println(testTitle)
	fmt.Println("Existing account owner name: ", testJointAccount.CheckName())
	fmt.Println("Changing account owner name to: ", newName)
	testJointAccount.ChangeOwnerName(newName)
	fmt.Println("→ Current account owner name: ", testJointAccount.CheckName())

	var testCustomer customer.Customer
	testCustomer.ChangeName("Test Customer")

	testTitle = "=== Test: Change a customer name on a Customer object"
	newName = "Second NewName"
	fmt.Println(testTitle)
	fmt.Println("Existing customer name: ", testCustomer.CheckName())
	fmt.Println("Changing customer name to: ", newName)
	testCustomer.ChangeName(newName)
	fmt.Println("→ Current customer name: ", testCustomer.CheckName())
}
